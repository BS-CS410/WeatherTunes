...
test.describe("Authentication System - Comprehensive Testing", () => {
  ...
  test.describe("Authentication Flow", () => {
    test("should handle complete OAuth PKCE flow correctly", async ({ page }) => {
    ...
    test("should handle OAuth callback correctly", async ({ page }) => {
      ...
      await page.goto(`${FRONTEND_URL}/callback?code=${mockCode}&state=${mockState}`);
      ...
      const hasError = await page.locator("h2:has-text('Authentication Error')").isVisible();
      const hasSuccess = await page.locator("text=/success|redirecting/i").isVisible();
      const hasProcessing = await page.locator("text=/authenticating/i").isVisible();
  ...
  test.describe("Authentication State Management", () => {
    test("should properly initialize authentication state", async ({ page }) => {
      await page.goto(FRONTEND_URL);
    ...
    test("should handle localStorage corruption gracefully", async ({ page }) => {
      ...
      const tokens = await page.evaluate(() => localStorage.getItem("spotify_tokens"));
  ...
  test.describe("Token Management", () => {
    ...
    test("should prevent concurrent token refresh attempts", async ({ page }) => {
      await page.goto(FRONTEND_URL);
