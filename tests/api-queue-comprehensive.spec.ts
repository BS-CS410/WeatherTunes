...
test.describe("API and Queue System - Comprehensive Testing", () => {
  ...
  test.describe("Spotify API Integration", () => {
    test("should make authenticated API requests correctly", async ({
      page,
    }) => {
      // Set up mock tokens
      ...
      const searchButton = page
        .locator("button:has-text('Search'), input[placeholder*='search' i]")
        .first();
      if (await searchButton.isVisible()) {
        ...
        const spotifyRequests = apiRequests.filter((req) =>
          req.url.includes("api.spotify.com"),
        );
        if (spotifyRequests.length > 0) {
          const authHeaders = spotifyRequests.filter(
            (req) =>
    ...
    test("should handle API rate limiting gracefully", async ({ page }) => {
      ...
      const hasErrorHandling = await page
        .locator("text=/rate.*limit|try.*again|error/i")
    ...
    test("should handle network errors gracefully", async ({ page }) => {
      ...
      const hasErrors = await page
        .locator("text=/network.*error|connection.*failed/i")
  ...
  test.describe("Queue Management", () => {
    ...
    test("should handle queue operations without authentication errors", async ({
      page,
    }) => {
      await page.evaluate((tokens) => {
        localStorage.setItem("spotify_tokens", JSON.stringify(tokens));
      ...
      const addButtons = page.locator(
        "button:has-text('Add'), button:has-text('Play'), [role='button']:has-text('+')",
      );
      ...
      if (buttonCount > 0) {
        ...
        const hasAuthErrors = await page
          .locator("text=/auth.*error|login.*required/i")
    ...
    test("should persist queue state across page reloads", async ({ page }) => {
      ...
      const initialQueueState = await page.evaluate(() => {
        const queueKeys = Object.keys(localStorage).filter(
          (key) =>
            key.toLowerCase().includes("queue") ||
            key.toLowerCase().includes("track"),
        );
        return queueKeys.map((key) => ({
          key,
  ...
  test.describe("Weather-Based Music Recommendations", () => {
    test("should generate weather-appropriate music recommendations", async ({
      page,
    }) => {
      await page.evaluate((tokens) => {
        localStorage.setItem("spotify_tokens", JSON.stringify(tokens));
      ...
      const weatherElements = await page
        .locator("text=/weather|clear|sunny|rainy|cloudy/i")
        .count();
      const musicElements = await page
        .locator("text=/music|song|track|artist/i")
        .count();
    ...
    test("should handle weather API failures gracefully", async ({ page }) => {
      ...
      expect(
        hasButtons + hasInputs + hasClickableElements,
      ).toBeGreaterThanOrEqual(0);
    });
  });
  ...
  test.describe("Error Recovery and Resilience", () => {
    ...
    test("should maintain app stability under rapid user interactions", async ({
      page,
    }) => {
      await page.evaluate((tokens) => {
        localStorage.setItem("spotify_tokens", JSON.stringify(tokens));
