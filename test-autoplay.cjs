...
filesToCheck.forEach((filePath) => {
  ...
  try {
    ...
    const spotifyEmbedRegex = /https:\/\/open\.spotify\.com\/embed\/track\/[^"]+/g;
    ...
    if (embedUrls.length === 0) {
      ...
      embedUrls.forEach((url, index) => {
        ...
        console.log(`   🎬 Autoplay enabled: ${hasAutoplay ? '✅ YES' : '❌ NO'}`);
...
if (allTestsPassed) {
  ...
  console.log('🎵 Music will start playing automatically when tracks are loaded');
} else {
