...
export type WeatherType = 'clear' | 'clouds' | 'rain' | 'snow' | 'fog' | 'thunderstorm' | 'drizzle';
...
export class WeatherService {
  ...
  private processForecastData(
    ...
    return sortedEntries.map(([dateKey, dayData]) => {
      ...
      const dayName = dateKey === todayDateKey 
        ? 'Today' 
      ...
      return {
        ...
        condition: primaryCondition.description
          .charAt(0)
          .toUpperCase() + primaryCondition.description.slice(1),
  ...
  public getWeatherType(condition?: string): WeatherType {
    ...
    if (lowerCondition.includes('drizzle') || lowerCondition.includes('shower')) {
      return 'drizzle';
    }
  ...
  public getVideoSource(
    ...
    return defaultVideoMap[weatherType]?.[period] || defaultVideoMap[this.DEFAULT_WEATHER][this.DEFAULT_TIME_PERIOD];
  }
}
